package com.dms.api.modules.controller.sys.menu;
...
public class SysMenuController extends AbstractController {
	...
	@RequestMapping("/nav")
	public R nav(){
		List<MenuEntity> router = sysMenuService.getUserMenuList(getUser());
		JSONObject data = new JSONObject(){{put("router", router);}};
	...
	public R delete(@RequestBody Integer menuId){
		...
		//判断是否有子菜单或按钮
		List<MenuEntity> menuList = sysMenuService.queryListParentId(menuId.longValue());
		if(menuList.size() > 0){
			return R.error("请先删除子菜单或按钮");
		}

		sysMenuService.deleteBatch(new Long[]{menuId.longValue()});
	...
	public List<SysMenuEntity> roleMenu() {
		//系统管理员，拥有最高权限
		if(getUser().isSysSuperAdmin()){
			return sysMenuService.queryList(null);
		}
		//用户对应角色可分配的权限菜单列表
		return sysMenuService.queryUserMenuList(getUserId());
	}
	...
	private void verifyForm(SysMenuEntity menu){
		if(StringUtils.isBlank(menu.getName())){
			throw new DMException("菜单名称不能为空");
		}
		
		if(menu.getParentId() == null){
			throw new DMException("上级菜单不能为空");
		}
		
		//菜单
		if(menu.getType() == Constant.MenuType.MENU.getValue()){
			if(StringUtils.isBlank(menu.getUrl())){
				throw new DMException("菜单URL不能为空");
			}
		}
		...
		if(menu.getType() == Constant.MenuType.CATALOG.getValue() ||
			...
			if(parentType != Constant.MenuType.CATALOG.getValue()){
				throw new DMException("上级菜单只能为目录类型");
			}
			return ;
		}
		...
		if(menu.getType() == Constant.MenuType.BUTTON.getValue()){
			if(parentType != Constant.MenuType.MENU.getValue()){
				throw new DMException("上级菜单只能为菜单类型");
			}
			return ;
		}
	}
