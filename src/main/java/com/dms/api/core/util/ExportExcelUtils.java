...
/**
 * @author jp
 * @Description: 非注解式excel导出
 * @date 2018/12/28 14:11
 */
public class ExportExcelUtils {
     ...
     * @param list 要导出的数据集合
     * @param formMap 中英文字段对应Map，即要导出的excel表头
     * @param response 使用response可以导出到浏览器
     * @param <T>
     */
    public static <T> void export(String excelName, List<T> list, LinkedHashMap<String, String> formMap, HttpServletResponse response){

        // 设置默认文件名为当前时间：年月日时分秒
        excelName = StringUtils.isBlank(excelName) ? new SimpleDateFormat("yyyyMMddhhmmss").format(new Date()).toString() : excelName;
        ...
        try(OutputStream os = response.getOutputStream()) {
            ...
            // 填充工作表
            fillSheet(sheet, list, formMap, wb);
            //将文件输出
...
//        logger.info("根据带路径或不带路径的属性名获取属性值,即接受简单属性名:getFieldValueByNameSequence()");
     ...
     * @throws Exception 异常
     */
    public static <T> void fillSheet(HSSFSheet sheet, List<T> list, LinkedHashMap<String, String> fieldMap, HSSFWorkbook wb) throws Exception {
//        logger.info("向工作表中填充数据:fillSheet()");
        ...
        row.setHeight((short) 450);      //设置行高50px
        // 表头样式设置
        ...
        for (int i = 0; i < cnFields.length; i++) {
            ...
            sheet.setColumnWidth(i,5200);   //设置列宽
        }
